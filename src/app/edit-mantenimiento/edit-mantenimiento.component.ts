...
export class EditMantenimientoDialogComponent {
    ...
    save(){
        ...
        this.mantenimientosService.updateMantenimiento(this.mantenimiento.eid, this.mantenimiento.id, changes)
          .subscribe(() => {
            const data = {
                equipoId: this.mantenimiento.id,
                fecha: new Date().toLocaleDateString(),
                hora: new Date().toLocaleTimeString(),
              };
          
              this.mantenimientosService.createRegistroM(data).then(
                (docRef) => {
                  console.log('Registro creado con ID:', docRef.id);
                  // Aquí puedes realizar más acciones después de crear el registro si es necesario
                },
                (error) => {
